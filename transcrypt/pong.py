...
class Ball(Sprite):
    ...
    def speedUp(self, bat):
        factor = 1 + 0.15 * (1 - Math.abs(self.y - bat.y) / (
                    bat.height // 2)) ** 2  # Speed will increase more if paddle hit near centre
...
class Game:
    def __init__(self):
        ...
        self.canvas = __new__(
            fabric.Canvas('canvas', {'backgroundColor': 'black', 'originX': 'center', 'originY': 'center'}))
